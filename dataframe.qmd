...
[-@data-structure] 장에서 R의 모든 기본 자료형과 자료구조에 대한 여행을 마쳤다.
데이터 과학에서 수행하는 모든 작업은 [-@data-structure] 장에서 소개한 도구를 조작하는 것이 된다.
하지만, 거의 대부분 데이터 과학 프로젝트에서 진정한 스타는 **데이터프레임(dataframe)**이다. 
CSV 파일에 정보를 불러와서 생성시킨 테이블을 지칭한다. 
이번 장에서 데이터프레임으로 작업하는 방법에 대해 좀 더 상세히 살펴볼 것이다.
...
3. `as.numeric()` 함수를 사용해서 `human_age` 벡터를 다시 숫자형 벡터로 변환시킨다.
...
데이터를 불러 읽어올 때 `stringsAsFactors=FALSE` 설정을 잊지 말자.
...
쓰레기 행을 추가시켰다. 데이터프레임에 문제가 되는 행을 마이너스 연산자를 사용해서 빼자.
...
항상 가장 먼저 해야 되는 작업은 `str` 명령어로 데이터가 어떻게 생겼는지 확인하는 것이다.
...
하지만, 이번에는 다르다. 데이터프레임은 **벡터와 요인으로 구성된 리스트**라는 사실을 기억하라.
...
문제를 해결하는 데, R이 데이터를 이해하는 방법과 데이터를 기록할 때 **엄격한 일관성(strict consistency)**의 중요성에 관해 학습한 것을 동원한다.
...
지금까지 학습한 요인, 리스트와 벡터 뿐만 아니라, `colnames()`, `dim()` 같은 함수도 동원한다.
